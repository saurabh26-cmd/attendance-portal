...
# Security: use a strong Render environment secret. If one is not configured,
# generate a random one so the app never falls back to a public/default secret.
app.secret_key = os.environ.get("SECRET_KEY") or os.urandom(32).hex()
app.config.update(
    SESSION_COOKIE_HTTPONLY=True,
    SESSION_COOKIE_SECURE=True,      # Render serves the app over HTTPS
    SESSION_COOKIE_SAMESITE="Lax",
    PERMANENT_SESSION_LIFETIME=1800, # 30 minutes
)
DB = os.environ.get("DB_FILE", "attendance.db")

# Admin credentials are supplied through Render Environment Variables.
# Do NOT put a real password in GitHub.
ADMIN_LOGIN = os.environ.get("ADMIN_LOGIN", "admin")
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD")

# Small in-memory login throttle. This resets when the service restarts.
LOGIN_WINDOW = 300
...
def init():
    ...
    if not admin:
        ...
        # Keep the admin password controlled by Render's secret environment
        # variable. This also replaces any old/default password in an existing DB.
        c.execute(
            "UPDATE users SET password_hash=?, role='admin', location='ALL', active=1 WHERE login_id=?",
            (generate_password_hash(ADMIN_PASSWORD), ADMIN_LOGIN)
        )
    c.commit()
    c.close()
...
def html(title, body):
    ...
    <a href="/monthly">Attendance</a><a href="/report">Reports</a><form method="post" action="/logout" style="display:inline"><input type="hidden" name="_csrf" value="{{ token }}"><button style="background:none;padding:0;margin-left:12px;color:white">Logout</button></form>
    {% endif %}</div></div>"""
...
def security_checks():
    ...
    # Never cache authenticated pages in the browser/proxy.
    if session.get("uid"):
        # Flask's after_request is used for the final response headers.
        pass
...
def security_headers(response):
    ...
    # Basic CSP suitable for this app's inline HTML/CSS.
...
def login_required(f):
    ...
    def w(*a, **k):
        if "uid" not in session:
            return redirect("/")
        # Re-check the account on every protected request so a disabled user
        # cannot continue using an already-issued session.
        c = db()
        u = c.execute(
...
def login():
    ...
    if request.method == "POST":
        ...
        if u and check_password_hash(u["password_hash"], password):
            ...
            session.update(
                ...
                location=u["location"], _csrf=csrf_token()
            )
            return redirect("/dashboard")
    ...
    <label>Login ID<input name="login_id" autocomplete="username" required></label><br>
    <label>Password<input type="password" name="password" autocomplete="current-password" required></label><br>
    <button>Login</button></form>
    </div>"""
    return page("Login", body, token=token)
...
def users():\n    check_csrf()
    ...
    <div class="card"><form method="post" class="grid"><input type="hidden" name="_csrf" value="{{ token }}">
...
def monthly():\n    check_csrf()
    ...
    <form method="post"><input type="hidden" name="_csrf" value="{{ token }}"><div class="card scroll"><table>
