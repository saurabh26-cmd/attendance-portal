...
.nav a{color:white;text-decoration:none;margin-left:12px;font-size:14px}.wrap{max-width:1400px;margin:24px auto;padding:0 16px}
...
input,select,button{padding:10px;border:1px solid #ccd3dd;border-radius:7px;font:inherit;width:100%}
...
.small{font-size:12px;color:#667085}.login{max-width:420px;margin:70px auto}.hero{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap}
.actions{display:flex;gap:8px;flex-wrap:wrap}.flash{background:#e7f7ef;padding:11px;border-radius:7px;margin-bottom:10px}
</style>"""
...
def html(title, body):
    ...
    <title>"""+title+"""</title>"""+CSS+"""</head><body>"""+nav+"""<div class="wrap">"""+body+"""</div></body></html>"""
...
def page(title, body):
    ...
    return render(html(title, body.replace("{{FLASH}}","".join("<div class='flash'>"+m+"</div>" for m in msgs))))
...
def admin_required(f):
    ...
    def w(*a,**k):
        if session.get("role")!="admin":
            flash("Admin access required."); return redirect("/dashboard")
        return f(*a,**k)
    return w
...
def login():
    if request.method=="POST":
        c=db()
        u=c.execute("SELECT * FROM users WHERE login_id=? AND active=1",
                    (request.form["login_id"].strip(),)).fetchone()
        c.close()
...
def dashboard():
    ...
    if session["role"]=="admin":
        ...
        emp=c.execute("SELECT COUNT(*) n FROM employees WHERE active=1 AND location=?",
                      (session["location"],)).fetchone()["n"]
        mgr=0; loc=session["location"]
    c.close()
    ...
    if request.method=="POST":
        try:
            c.execute("""INSERT INTO users(name,login_id,password_hash,role,location)
                      VALUES(?,?,?,?,?)""",
                      (request.form["name"].strip(),request.form["login_id"].strip(),
                       generate_password_hash(request.form["password"]),
                       "manager",request.form["location"].strip()))
            c.commit(); flash("Manager login created.")
        except sqlite3.IntegrityError: flash("Login ID already exists.")
    users=c.execute("SELECT name,login_id,role,location,active FROM users ORDER BY name").fetchall()
    locs=c.execute("SELECT DISTINCT location FROM employees WHERE location<>'' ORDER BY location").fetchall()
    c.close()
    opts="".join(f"<option>{x['location']}</option>" for x in locs)
    rows="".join(f"<tr><td>{u['name']}</td><td>{u['login_id']}</td><td>{u['role']}</td><td>{u['location']}</td><td>{'Active' if u['active'] else 'Inactive'}</td></tr>" for u in users)
    body=f"""{{{{FLASH}}}}<h1>Manager Accounts</h1>
    <div class="card"><form method="post" class="grid">
    <input name="name" placeholder="Manager name" required><input name="login_id" placeholder="Login ID" required>
    <input name="password" placeholder="Password" required><select name="location" required>{opts}</select>
    <button>Create Manager</button></form></div>
    <div class="card scroll"><table><tr><th>Name</th><th>Login ID</th><th>Role</th><th>Location</th><th>Status</th></tr>{rows}</table></div>"""
...
def employees():
    c=db()
    if request.method=="POST":
        c.execute("""INSERT INTO employees(employee_code,name,designation,location)
                  VALUES(?,?,?,?)""",
                  (request.form["code"].strip(),request.form["name"].strip(),
                   request.form["designation"].strip(),request.form["location"].strip()))
        c.commit(); flash("Employee added.")
    emps=c.execute("SELECT * FROM employees WHERE active=1 ORDER BY location,name").fetchall(); c.close()
    rows="".join(f"<tr><td>{e['employee_code']}</td><td>{e['name']}</td><td>{e['designation']}</td><td>{e['location']}</td></tr>" for e in emps)
    body=f"""{{{{FLASH}}}}<h1>Employee Master</h1>
    <div class="card"><form method="post" class="grid">
    <input name="code" placeholder="Employee ID" required><input name="name" placeholder="Employee Name" required>
    <input name="designation" placeholder="Designation" required><input name="location" placeholder="Warehouse / Location" required>
    <button>Add Employee</button></form></div>
    <div class="card scroll"><table><tr><th>Employee ID</th><th>Name</th><th>Designation</th><th>Location</th></tr>{rows}</table></div>"""
...
def monthly():
    ...
    if not locs: c.close(); return page("Attendance","<div class='card'>No location assigned.</div>")
    ...
    if request.method=="POST":
        ...
        c.commit(); c.close(); flash(f"Attendance saved for {calendar.month_name[month]} {year}.")
    ...
    for e in emps:
        data[e["id"]]={r["att_date"]:r["status"] for r in c.execute(
            ...
            (e["id"],start,end))}
    c.close()
    opts_loc="".join(f"<option {'selected' if x==location else ''}>{x}</option>" for x in locs)
    ...
    for e in emps:
        ...
        for d in range(1,days+1):
            ...
            opts="<option value=''></option>"+''.join(
                f"<option value='{s}' {'selected' if cur==s else ''}>{s}</option>" for s in ["P","A","L","WO"])
    ...
    <div class="actions"><a class="btn secondary" href="/report?location={location}&year={year}&month={month}">Report</a>
    <a class="btn success" href="/export?location={location}&year={year}&month={month}">Export CSV</a></div></div>
    <form method="get" class="card grid"><label>Warehouse<select name="location">{opts_loc}</select></label>
    ...
    <form method="post"><div class="card scroll"><table><tr><th class="emp">Employee</th>{heads}</tr>{rows}</table></div>
...
def report():
    ...
    for e in emps:
        ...
        for r in c.execute("SELECT status FROM attendance WHERE employee_id=? AND att_date BETWEEN ? AND ?",
    ...
    c.close()
    body=f"""<div class="hero"><div><h1>Attendance Report</h1><p class="small">{calendar.month_name[month]} {year} · {location}</p></div>
    <a class="btn success" href="/export?location={location}&year={year}&month={month}">⬇ Download CSV</a></div>
    <div class="card scroll"><table><tr><th>Employee ID</th><th>Name</th><th>Designation</th><th>Present</th><th>Absent</th><th>Leave</th><th>Weekly Off</th><th>Total</th></tr>{rows}</table></div>"""
...
def export():
    ...
    w.writerow(["Employee ID","Name","Designation","Location"]+[str(d) for d in range(1,days+1)]+["Present","Absent","Leave","WO"])
    for e in emps:
        sts=[]; cnt={s:0 for s in ["P","A","L","WO"]}
        for d in range(1,days+1):
            r=c.execute("SELECT status FROM attendance WHERE employee_id=? AND att_date=?",
                        (e["id"],f"{year:04d}-{month:02d}-{d:02d}")).fetchone()
            s=r["status"] if r else ""; sts.append(s)
            if s in cnt: cnt[s]+=1
        w.writerow([e["employee_code"],e["name"],e["designation"],e["location"]]+sts+
                   [cnt["P"],cnt["A"],cnt["L"],cnt["WO"]])
    c.close(); out.seek(0)
